...
```{r}
# data cleaning, we are going to drop some vars
#final_data <- drop_columns(final_data, c("dst_origin", "tzone_origin", "year_flights", "tz_origin"))

df_clean = df
df_clean = df %>% drop_columns(c("Inspection.ID", "License.#", "Address", "Violations", "Location"))
...
df_clean = droplevels(filter(df_clean, City  %in% c("Chicago","312CHICAGO", "CCHICAGO", 
                                                    "CHCHICAGO","CHCICAGO","chicago","Chicago", "CHicago", "CHICAGO", 
                                                    "CHICAGOC","CHICAGOCHICAGO", "CHICAGOI","CHICAGOO", "312CHICAGO")))
...
df_clean2$Facility.Type  = df_clean2$Facility.Type%>% forcats::fct_lump_min(min = 200, other_level = "Other") 
...
levels(df_clean2$Facility.Type)

#save grouped data to variable 
bar = df_clean2 %>% group_by(Facility.Type) %>% summarise(count = length(Facility.Type)) %>%
             ungroup() %>% arrange(desc(count))
...
bar2 = df_clean2 %>% group_by(Facility.Type) %>% summarise(count = n()) %>% 
...
bar4 = df_clean2[df_clean2$Facility.Type == "Restaurant",] %>%
...
ggplot(pct_table, aes(x=YearofInspection)) + 
  geom_line(aes(y = pct_table$Fail, color = "red")) + 
  geom_line(aes(y = pct_table$Pass, color="darkgreen")) + 
  geom_line(aes(y = pct_table$`Pass w/ Conditions`, color = "darkgoldenrod1")) + 
...
df_clean2$yearIns = (year(df_clean2$Inspection.Date))

zipdf = filter(df_clean2, df_clean2$yearIns %in%
                    c("2022", "2021"))
...
ggplot(bottom10_zip, aes(x = zip_list, y = bottom10_zip$Fail, fill = zip_list)) + geom_col() +
...
prop_df = filter(df_clean2, df_clean2$yearIns %in% c("2021"), df_clean2$Facility.Type == "Restaurant" )
...
df_clean2$Results  = recode_factor(df_clean2$Results, "Pass w/ Conditions" = "Pass")

prop_df2 = filter(df_clean2, df_clean2$yearIns %in% c("2021"), df_clean2$Facility.Type == "Restaurant" )
